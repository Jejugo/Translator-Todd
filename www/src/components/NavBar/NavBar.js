...
export default class NavBar extends Component {
    ...
    componentDidUpdate(prevProps, prevState){
        if(this.props.show !== prevProps.show){
            ...
            show === true ? this.setState({navbarClass: 'navbar'}) : this.setState({navbarClass: 'navbar-stick'}) 
        }
    }
    ...
    render() {
        ...
        const navLinks = navOptions.map(item => {
            return(
                <Fragment>
                    <li className="navbar__content_list--item"><a href="#" className="navbar__content_list--link" onClick={() => this.scroll(item.alias)}>{item.name}</a></li>
                </Fragment>
            )
        });
